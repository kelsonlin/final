...
def save_to_record(images_addr, labels, files_name, bboxs, cats, option = None): 
	...
	for i in range(length):
		...
		(h, w) = image.shape[:2]
		split = images_addr[i].split(".jpg")
                print split[0]
		with tf.gfile.GFile(PATH_TO_FILES+ split[0] + "revised.jpg", 'rb') as fid:
			encoded_jpg = fid.read()
		label_num = labels[i]
		example = tf.train.Example(features=tf.train.Features(feature={
			'image/height': int64_feature(SIZE),
  			'image/width': int64_feature(SIZE),
